...
import { Avatar, Badge, Button, Container, Divider, Flex, FormControl, FormErrorMessage, FormLabel, IconButton, Input, Select, Td, Text, Tr, useDisclosure, useToast } from '@chakra-ui/react'
...
const MoreModal = ({ user }) => {
	...
	return (
		<Modal title="Edit User" toggle={(onOpen) => <IconButton size="xs" icon={<FiMoreHorizontal size={12} />} onClick={() => clearErrors() || reset() || onOpen()} />} disclosure={disclosure}>
			<form onSubmit={handleSubmit(onSubmit)}>
				<Flex direction="column" gap={6}>
					...
					<FormControl>
						...
						<Select defaultValue={user.status} size="lg" {...register('status', { required: true })}>
			...
			<Card>
				<Flex direction="column" gap={6}>
					...
					<Table
						...
						th={['Full Name', 'Email', 'Role', 'Status', 'Joined', '']}
						td={(user) => (
							<Tr key={user._id}>
								<Td>
									<Flex align="center" gap={3}>
										<Avatar name={user.name} src={user.image} />
										<Text color="accent-1">{user.name}</Text>
									</Flex>
								</Td>
								...
								<Td>
									<Badge variant="tinted" colorScheme={user.role === 'Admin' ? 'yellow' : 'brand'}>
										{user.role}
									</Badge>
								</Td>

								<Td>
									<Badge variant="tinted" colorScheme={user.status === 'active' ? 'brand' : 'red'} textTransform="capitalize">
										{user.status}
									</Badge>
								</Td>
