...
import { Button, Container, Divider, Flex, FormControl, FormErrorMessage, FormLabel, IconButton, Input, Menu, MenuButton, MenuItem, MenuList, Td, Text, Tr, useDisclosure, useToast } from '@chakra-ui/react'
	...
	const addServices = useMutation((data) => api.create('/services', data), {
		onSuccess: () => {
			...
			toast({
				position: 'top',
				duration: 1000,
				render: () => <Toast title="Success" description="Services added successfully." />
			})
		}
	})
...
const EditModal = ({ service }) => {
	...
	const editServices = useMutation((data) => api.update('/services', service._id, data), {
		onSuccess: () => {
			...
			toast({
				position: 'top',
				duration: 1000,
				render: () => <Toast title="Success" description="Service updated successfully." />
			})
		}
	})
		...
		>
			<form onSubmit={handleSubmit(onSubmit)}>
				<Flex direction="column" gap={6}>
					<FormControl isInvalid={errors.name}>
						<FormLabel>Name</FormLabel>
						<Input defaultValue={service.name} size="lg" {...register('name', { required: true })} />
						<FormErrorMessage>This field is required.</FormErrorMessage>
					</FormControl>

					<FormControl isInvalid={errors.price}>
						<FormLabel>Price</FormLabel>
						<Input defaultValue={service.price} size="lg" {...register('price', { required: true })} />
						<FormErrorMessage>This field is required.</FormErrorMessage>
					</FormControl>
...
const Services = () => {
	...
	const deleteServices = useMutation((data) => api.remove('/services', data), {
		onSuccess: () => {
			queryClient.invalidateQueries('services')
			...
			toast({
				position: 'top',
				duration: 1000,
				render: () => <Toast title="Success" description="Services removed successfully." />
			})
		}
	})
					...
					<Table
						...
						td={(service) => (
							<Tr key={service._id}>
								...
								<Td>
									<Text>{service.price.toLocaleString(undefined, { maximumFractionDigits: 2 })}</Text>
								</Td>
								...
								<Td textAlign="right">
									<Menu placement="bottom-end">
										...
										<MenuList>
											...
											<MenuItem icon={<FiTrash2 size={16} />} onClick={() => onSubmit(service._id)}>
												Delete
											</MenuItem>
										</MenuList>
									</Menu>
